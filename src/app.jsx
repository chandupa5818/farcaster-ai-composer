...
const generateContentPlan = async (topic, style = 'unhinged') => {
  ...
  const systemPrompt = `
    ...
    1. Write a draft post (max 280 chars) DIRECTLY RELATED to the topic.
       - ${styleInstruction}
       - AVOID: Hashtags (unless ironic), corporate speak, "AI language" (like "delve", "testament").
...
const regenerateText = async (topic, style) => {
  let stylePrompt = "";
  if (style === 'professional') {
    stylePrompt = "Write a professional, crisp, and engaging post. Use proper grammar. Aim for a tech-thought-leader vibe. No hashtags.";
  } else if (style === 'funny') {
    stylePrompt = "Write a witty, clever, and humorous post. Make a joke related to the topic. Casual tone.";
  } else if (style === 'unhinged') {
    stylePrompt = "Write a chaotic, lowercase, 'shitpost' style post. Use internet slang, be absurd, maybe slightly dramatic or self-deprecating.";
  }
  ...
  const systemPrompt = `
    ...
    Return ONLY the plain text of the post. Do not include JSON formatting.
  `;
...
const fetchImage = async (searchTerm) => {
  // Option A: Real Unsplash API (Requires Key)
  if (UNSPLASH_ACCESS_KEY) {
    try {
      const response = await fetch(
        `https://api.unsplash.com/search/photos?page=1&query=${encodeURIComponent(searchTerm)}&per_page=1&orientation=squarish`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        if (data.results && data.results.length > 0) {
          return {
            preview: data.results[0].urls.small, 
            full: data.results[0].urls.regular   
          };
        }
      }
    } catch (e) {
...
const ImageOption = ({ imgData, label, selected, onClick, loading, isCustom, onUpload }) => {
  if (isCustom && !imgData) {
    return (
      <button
        onClick={onUpload}
        className={`relative group overflow-hidden rounded-xl aspect-square border-2 border-dashed border-slate-700 hover:border-purple-500 hover:bg-slate-800/50 transition-all duration-300 w-full flex flex-col items-center justify-center gap-2 ${
          selected ? 'border-purple-500 bg-slate-800' : ''
        }`}
      >
  ...
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className={`relative group overflow-hidden rounded-xl aspect-square border-2 transition-all duration-300 w-full ${
        selected 
    ...
    >
      {loading ? (
        ...
        <>
          <img 
            src={isCustom ? imgData : imgData.preview} 
            alt={label} 
            className="object-cover w-full h-full transition-transform duration-500 group-hover:scale-110" 
          />
          {!isCustom && (
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <p className="text-[10px] text-white truncate text-center">{label}</p>
            </div>
          )}
          {selected && (
...
const StyleButton = ({ active, onClick, icon: Icon, label, colorClass, borderColorClass }) => (
  <button
    onClick={onClick}
    className={`flex-1 flex flex-col items-center justify-center gap-2 py-3 rounded-xl border-2 transition-all duration-300 ${
      active 
        ? `${borderColorClass} bg-slate-800 shadow-lg scale-105` 
...
export default function App() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.warn("Farcaster SDK not detected or failed to load.", e);
      }
    };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(err);
      setError("The AI tripped over a wire. Try again.");
      setStep("input");
    }
  };
  ...
  const copyTextToClipboard = () => {
    ...
    alert("Text copied! Go be famous.");
  };

  const copyImageLink = () => {
    if (selectedImgIndex === 3) {
      alert("This is your local file! You can just attach it directly from your gallery in Warpcast.");
      return;
    }
    const imgData = generatedImages[selectedImgIndex];
    if (imgData && imgData.full) {
        navigator.clipboard.writeText(imgData.full);
        alert("High-Quality Image URL copied!");
    }
  };
    ...
    <div className="min-h-screen bg-[#0f1117] text-slate-100 font-sans selection:bg-purple-500/30">
      <div className="max-w-md mx-auto min-h-screen flex flex-col relative bg-[#15171e] shadow-2xl overflow-hidden">
        ...
        <main className="flex-1 p-6 flex flex-col relative">
          ...
          {step === 'input' && (
            <div className="flex flex-col h-full justify-center animate-in slide-in-from-bottom-4 duration-500">
              <div className="mb-6 text-center space-y-2">
                ...
                <p className="text-slate-400">Give me a topic, I'll find the perfect vibe.</p>
              </div>
              ...
              <div className="relative group mb-6">
                <textarea
                  ...
                  placeholder="e.g., why my code is broken, crypto twitter drama, eating cereal at 3am..."
                  className="w-full h-32 bg-slate-900 border border-slate-700 rounded-2xl p-4 text-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all resize-none placeholder:text-slate-600 shadow-inner"
                />
              ...
              <div className="mb-6">
                <p className="text-sm text-slate-500 mb-3 ml-1 font-medium">Select Tone:</p>
                <div className="flex gap-3">
                  <StyleButton 
                    active={selectedStyle === 'professional'} 
                  ...
                  />
                  <StyleButton 
                    active={selectedStyle === 'unhinged'} 
                    onClick={() => setSelectedStyle('unhinged')}
                    icon={Zap}
                    label="Unhinged"
              ...
              <button
                ...
                disabled={!topic.trim()}
                className="w-full bg-white text-black py-4 rounded-xl font-bold text-lg hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-[0_0_30px_rgba(255,255,255,0.2)] flex items-center justify-center gap-2"
              >
              ...
              <div className="mt-6 text-center">
                 <p className="text-[10px] text-slate-600 uppercase tracking-wider">
                   Powered by Gemini & {UNSPLASH_ACCESS_KEY ? "Unsplash" : "Pollinations AI"}
                 </p>
              </div>
            </div>
          )}
          ...
          {step === 'generating' && (
            <LoadingStep status={loadingStatus} />
          )}
          ...
          {step === 'preview' && (
            <div className="flex flex-col space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <textarea
                    ...
                    className={`w-full bg-slate-900/50 border border-slate-700 rounded-xl p-4 text-base focus:outline-none focus:border-purple-500 transition-colors resize-none h-28 ${isRegeneratingText ? 'opacity-50' : ''}`}
                  />
                  {isRegeneratingText && (
                ...
                <div className="flex gap-2">
                  <button 
                    onClick={() => handleRegenerateText('professional')}
                    disabled={isRegeneratingText}
                    className={`flex-1 text-xs py-2 rounded-lg transition-colors border ${selectedStyle === 'professional' ? 'bg-blue-500/20 border-blue-500 text-blue-300' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}
                  >
                    👔 Pro
                  </button>
                  <button 
                    onClick={() => handleRegenerateText('funny')}
                    disabled={isRegeneratingText}
                    className={`flex-1 text-xs py-2 rounded-lg transition-colors border ${selectedStyle === 'funny' ? 'bg-yellow-500/20 border-yellow-500 text-yellow-300' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}
                  >
                    😂 Funny
                  </button>
                  <button 
                    onClick={() => handleRegenerateText('unhinged')}
                    disabled={isRegeneratingText}
                    className={`flex-1 text-xs py-2 rounded-lg transition-colors border ${selectedStyle === 'unhinged' ? 'bg-purple-500/20 border-purple-500 text-purple-300' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}
                  >
                    🤪 Unhinged
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {generatedImages.map((imgData, idx) => (
                    <ImageOption
                      key={idx}
                      imgData={imgData} 
                      label={searchTerms[idx]}
                      selected={selectedImgIndex === idx}
                      onClick={() => setSelectedImgIndex(idx)}
                      loading={!imgData && generatedImages.length === 0} 
                      isCustom={false}
                    />
                  ))}
                  ...
                  <ImageOption
                    ...
                    imgData={customImage}
                    label="My Photo"
                    selected={selectedImgIndex === 3}
                    onClick={() => customImage ? setSelectedImgIndex(3) : triggerFileUpload()}
              ...
              <div className="pt-4 space-y-3 mt-auto">
                <button
                  onClick={handlePost}
                  className="w-full bg-[#855DCD] hover:bg-[#734eb5] text-white py-3.5 rounded-xl font-bold text-lg transition-all shadow-lg shadow-purple-900/20 flex items-center justify-center gap-2"
                >
            ...
            <div className="flex flex-col items-center justify-center h-full text-center space-y-6 animate-in zoom-in duration-300">
              {step === 'posting' ? (
                ...
                <>
                  ...
                  <h3 className="text-2xl font-bold text-white">Banger Secured!</h3>
                  ...
                  <div className="flex flex-col w-full gap-3 mt-8">
                    <button 
                      ...
                      className="w-full bg-white text-black py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-200"
                    >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button 
                          ...
                          className="w-full bg-slate-800 text-white py-3 rounded-xl font-medium hover:bg-slate-700 text-sm"
                        >
                          Copy Text
                        </button>
                        <button 
                          onClick={copyImageLink}
                          className={`w-full bg-slate-800 text-white py-3 rounded-xl font-medium hover:bg-slate-700 text-sm flex items-center justify-center gap-2 ${selectedImgIndex === 3 ? 'opacity-50' : ''}`}
                        >
                          <LinkIcon className="w-3 h-3" />
                          {selectedImgIndex === 3 ? "Local File" : "Copy HD Image"}
                        </button>
                    </div>
                    <button 
                      onClick={reset}
                      className="text-slate-500 text-sm mt-4 hover:text-slate-300"
                    >
                      Make Another One
                    </button>
                  </div>
